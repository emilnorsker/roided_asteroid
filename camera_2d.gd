...
func _physics_process(_dt: float) -> void:
	...
	# World units visible in half-width/height = (view_size / 2) / zoom
	...
	# Use whichever is smaller (more zoom-out) to keep Sun on screen
	target_zoom = min(target_zoom, zoom_to_fit)

	# Ensure we stay within sane limits
	target_zoom = clamp(target_zoom, 0.1, max_zoom)
