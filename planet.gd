...
@export var orbit_center: NodePath           # The Sun (or other body)
...
func _ready():
	# Set visual size
	$Sprite2D.scale = Vector2.ONE * (planet_radius / 32.0)  # Assumes default 64px image
...
func _process(delta):
	...
	if orbit_center:
		var center = get_node(orbit_center).global_position
